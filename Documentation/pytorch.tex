...
The model is exported to ONNX format to allow real-time inference in the desktop application without running a full PyTorch environment in the frontend process. The exporter uses PyTorch 2.9/2.10+ Dynamo tracing and configures dynamic shapes for batch size and time dimensions. To avoid CP1252 print-encoding crashes on Windows, standard output is programmatically reconfigured to UTF-8. The model is exported to a single self-contained ONNX file by disabling external data:
...
\end{lstlisting}
The exported file is saved as a single self-contained \texttt{PyTorch.onnx} and executed using ONNX Runtime.
